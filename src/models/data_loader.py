...
class Batch(object):
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            # src, tgt, segs, clss, src_sent_labels, tgt_segs, tgt_eng, tgt_txt_eng
            ...
            # print("pre tgt eng = ", pre_tgt_eng)
            # print("pre tgt eng str = ", [x[-1] for x in data])

            src = torch.tensor(self._pad(pre_src, 0))
            tgt = torch.tensor(self._pad(pre_tgt, 0))

            tgt_eng = torch.tensor(self._pad(pre_tgt_eng, 0))

            segs = torch.tensor(self._pad(pre_segs, 0))
            if pre_tgt_segs[0] is not None:
                tgt_segs = torch.tensor(self._pad(pre_tgt_segs, 0))
            else:
                tgt_segs = None
            ...
            if (is_test):
                ...
                tgt_eng_str = [x[-3] for x in data]
                setattr(self, 'tgt_eng_str', tgt_eng_str)
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        if self.args.train_first is True:
            tgt = ex['tgt_eng'][:self.args.max_tgt_len][:-1] + [2]
        else:
        ...
        src_txt = ex['src_txt']
        tgt_txt = ex['tgt_txt']
        tgt_txt_eng = ex['tgt_txt_eng']
        tgt_segs = None
        if 'tgt_segs' in ex.keys() and not self.args.train_first:
            tgt_segs = ex['tgt_segs']
        ...
        if(is_test):
            ...
            return src, tgt, segs, clss, src_sent_labels, tgt_segs, tgt_eng, tgt_txt_eng
...
class TextDataloader(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        src_txt = ex['src_txt']
        if self.args.train_eng:
            tgt_txt = ex['tgt_txt_eng']
        else:
            tgt_txt = ex['tgt_txt']
        tgt_txt_eng = ex['tgt_txt_eng']
        tgt_segs = None
        if 'tgt_segs' in ex.keys():
            tgt_segs = ex['tgt_segs']
